  ...
  <div className='space-y-1.5 text-sm'>
    {(code.floor || code.instructions) && (
      <div className='flex items-start gap-2 text-[#1B2838]/80 dark:text-foreground/80'>
        <Building2 className='mt-0.5 h-3.5 w-3.5 shrink-0 text-[#1B2838]/50' aria-hidden />
        <p className='leading-snug'>
          {code.floor && <span className='font-medium'>{t('floor', { n: code.floor })}</span>}
          {code.floor && code.instructions && <span className='text-[#1B2838]/30'> · </span>}
          {code.instructions}
        </p>
      </div>
    )}
    {code.parking_hint && (
      <div className='flex items-start gap-2 text-[#1B2838]/80 dark:text-foreground/80'>
        <CarFront className='mt-0.5 h-3.5 w-3.5 shrink-0 text-[#1B2838]/50' aria-hidden />
